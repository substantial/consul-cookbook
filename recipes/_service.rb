...
when 'cluster'
  ...
  service_config['start_join'] = node['consul']['servers'] if num_cluster > 1
when 'server'
  service_config['server'] = true
...
copy_params = [
  :bind_addr, :datacenter, :domain, :log_level, :node_name, :advertise_addr, :ports, :enable_syslog, :skip_leave_on_terminate
]
